...
const CredentialModal = ({ title, onClose, children }: CredentialModalProps) => {
  ...
  return (
    <div ref={modalRef} className="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
      <div ref={backdropRef} className="absolute inset-0 bg-foreground/60 backdrop-blur-sm" onClick={handleClose} />
      <div ref={contentRef} className="relative bg-card/95 backdrop-blur-md rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden border border-border/40">
        <div className="flex items-start justify-between p-6 border-b border-border/50 bg-card/80 backdrop-blur-sm">
          <h2 className="text-2xl font-light text-foreground">{title}</h2>
          <button onClick={handleClose} className="p-2 hover:bg-muted/60 rounded-lg transition-colors shrink-0 mt-0.5">
